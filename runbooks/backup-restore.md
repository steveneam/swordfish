...
| 2026-07-15 | `584e14a1` | 6 s (215 files, 14M) | 0 h (minutes after apply-run backup) | **syd2 landing-zone exception drill** (migration plan Phase 0.2) — drill run 29406068782: the Project 1 corpus exclusion (`/srv/project1/assets/**`, recorded exception — reproducible reference data, re-seed = restore path) verified BOTH directions: `manifests/landing-drill.manifest` restored, zero files under `assets/`, canary described by the restored manifest; tenant-pg functional restore still green. Ran BEFORE any corpus data exists — backups-before-workloads held. Restic note: adding `/srv/project1` to the source list regrouped retention (`no parent snapshot found` once, old path-group snapshots persist in B2 under their own policy) |
