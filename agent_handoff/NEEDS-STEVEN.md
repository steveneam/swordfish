...
- [2026-07-13] One-time pass: correct `inventory/secrets/subscriptions.yml` — Claude plan price + next-charge dates are placeholders (billing APIs approved and live as of this session).
...
- [2026-07-13] E1 relay E2E: **PASSED** — thalon topic round-tripped live at 21:20 (your message injected, the thalon agent's real reply delivered back to the topic; ledger + terminal verified). Optional re-confirm of the Swordfish reply leg on the fixed hook: send any plain message in the Swordfish topic. Kill switch stays: `!kill` in the topic; poller backstop: `sudo systemctl stop swordfish-relay`.
...
- [2026-07-13] One tap: add `@Swordfish_alerts_bot` to the ops Telegram group (member, no admin needed). Relayed replies then arrive from that bot and hermes stops answering when you reply to a relayed message; until then the relay falls back to the hermes voice and everything works as today. (Asked in the Swordfish topic 21:45.)
