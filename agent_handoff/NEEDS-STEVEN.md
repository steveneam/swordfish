...
- [2026-07-28] 🔁 **Auto-reboot posture, fleet-wide — now a clean two-way call.** All three boxes carry `Automatic-Reboot true` + **`WithUsers true`** + `Automatic-Reboot-Time 18:30`. On syd3 that is cheap. On **syd4** it reboots *with users logged in*, killing live agent sessions; on **syd2** that same 18:30 reboot caused the 7h15m edge outage on 07-18. **The Kuma clause is RESOLVED 07-29: the alert path is proven end-to-end** (test alert reached your phone — your confirm), so a real outage WILL page you either way; the old option (c) is gone. Both boxes were attended-rebooted + verified outside-in on 07-29, so nothing is time-pressed. **(a)** leave as-is (it self-heals; auto-patching keeps its value; Kuma now proven to page you) · **(b)** keep auto-patching, disable auto-*reboot* on syd2+syd4 so reboots are deliberate — the two clean attended cycles on 07-29 are the working example. Config-only, no spend, reversible.
