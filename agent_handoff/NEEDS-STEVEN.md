...
- [2026-07-29] 🔑 **"re-issue thalon's deploy key" — one line, no spend.** Thalon cannot deploy their own staging app: their copy of the credential is dead (they re-tested, HTTP 401) and their other route, GitHub Actions, is billing-blocked. Swordfish's copy works (HTTP 200), so nothing is broken — but every thalon deploy currently routes through swordfish, and I ran one for them today. Handing over anything from `inventory/secrets/` is a rule-10 founder gate, which is why this waits on you. The credential is deploy-only: it cannot change env or config, so the blast radius is exactly "thalon can redeploy their own app". _Not urgent — it removes swordfish from the middle of their release loop._
...
- [2026-07-13] 📝 **3 fields in `inventory/secrets/subscriptions.yml`** (edit the file or tell any agent): Claude → which card; GitHub → next_charge date + which card, from the receipt. **The GitHub date is load-bearing** — it is the Actions refresh date, and swordfish will watch for the billing block to clear on that day and re-verify. _(Caveat worth knowing: the block says "payments failed OR spending limit"; if it is a failed payment, the refresh alone may not clear it.)_
